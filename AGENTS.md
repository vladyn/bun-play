...
- **60% test coverage required** -- Lines, branches, functions, and statements. Enforced via `.coverage-thresholds.json` as a blocking gate before PR creation and task completion
...
- Bun runtime with TypeScript source files
- No standalone linter, formatter, or type checker is currently configured
